...
function AppStateProvider({ children }) {
  ...
  // [{id, quantity:1(누른 횟수대로)}]
  const addToOrder = React.useCallback((id) => {
    console.log(id);

    setOrders((orders) => {
      // addToOrder에 인자로 전달한 id와 같은 order를 찾아서 finded에 넣는다.
      const finded = orders.find((order) => {
        return order.id === id;
      });

      // 이미 들어온 주문에서 id가 겹치는 주문이 없다면 새로 1로 해서 넣는다.
      if (finded === undefined) {
        ...
        return orders.map((element) => {
          if (element.id === id) {
            ...
            // 주문 요소들을 돌면서 한번씩 다 확인해줄 건데 id가 같지 않다면 그냥 원래 대로 가면 된다.
            return element;
          }
        });
      }
